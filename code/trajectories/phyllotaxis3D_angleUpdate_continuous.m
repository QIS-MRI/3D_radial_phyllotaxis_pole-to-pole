function [m_adPolarAngle_out, m_adAzimuthalAngle_out, x, y, z] = phyllotaxis3D_angleUpdate_continuous(m_lNumberOfFrames, m_lProjectionsPerFrame, flagSelf, nPC)
% PHYLLOTAXIS3D_ANGLEUPDATE_CONTINUOUS  Generates the continuous 3D phyllotaxis trajectory with different spokes per phase cycle (nPC). (2024)
%
%   [m_adPolarAngle_out, m_adAzimuthalAngle_out, x, y, z] = phyllotaxis3D_angleUpdate_continuous(m_lNumberOfFrames, m_lProjectionsPerFrame, flagSelf, nPC)
%
%   Inputs:
...
NProj = m_lNumberOfFrames * m_lProjectionsPerFrame ; % shots * segments
...
kost = pi/(2*sqrt(N/2)); % continuous phyllotaxis uses sqrt(N/2)
    ...
    for lk = 1:m_lProjectionsPerFrame % segments
        for lFrame = 1:m_lNumberOfFrames % shots
            ...
            if flagSelf && lk == 1
                ...
                % continuous phyllotaxis updating the polar angle between phase cycles (nPC)
                if (count - (N * (lpc-1)))<=(N/4) 
                    m_adPolarAngle(linter) = kost * sqrt(2*(count - (N * (lpc-1))));
                elseif (count - (N * (lpc-1)))>(N/4) && (count - (N * (lpc-1)))<=(N/2)
                    m_adPolarAngle(linter) = pi - kost * sqrt(2*(N/2-(count - (N * (lpc-1))))) ;
                elseif (count - (N * (lpc-1)))>(N/2) && (count - (N * (lpc-1)))<=(N/4*3)
                    m_adPolarAngle(linter) = pi +  kost * sqrt(2*((count - (N * (lpc-1)))-N/2)) ;
                else
                    m_adPolarAngle(linter) = 2*pi - kost * sqrt(2*(N-(count - (N * (lpc-1))))) ;
                end
