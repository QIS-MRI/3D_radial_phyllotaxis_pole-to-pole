...
% PHYLLOTAXIS3D_ANGLEUPDATE_POLETOPOLE  Generates the pole-to-pole 3D phyllotaxis trajectory with different spokes per phase cycle (nPC). (2024)
%
%   [m_adPolarAngle_out, m_adAzimuthalAngle_out, x, y, z] = phyllotaxis3D_angleUpdate_poletopole(m_lNumberOfFrames, m_lProjectionsPerFrame, flagSelf, nPC)
%
%   Inputs:
    ...
    for lk = 1:m_lProjectionsPerFrame % segments
        for lFrame = 1:m_lNumberOfFrames % shots
            ...
            if flagSelf && lk == 1
                ...
                % pole-to-pole phyllotaxis updating the polar angle between phase cycles (nPC)
                if (count - (N * (lpc-1)) )<=(N/2)
                    m_adPolarAngle(linter) = kost * sqrt(2*(count - (N * (lpc-1)) ));
                else
                    m_adPolarAngle(linter) =  pi - kost * sqrt(2*(N - (count - (N * (lpc-1)) )));
                end
